...
// For Baffin-TD, we need to turn on MIC-Bias, not enabling whole RX path. 
//
void csl_caph_audioh_mic_bias_on(Boolean on)
{
	aTrace(LOG_AUDIO_CSL, "%s, ON=1,OFF=0 ?: %d\n",
		__func__, on);
	if(on)
	{
		chal_audio_mic_pwrctrl(handle, TRUE);
...
void csl_caph_audioh_hs_mic_bias_on(Boolean on)
{
	aTrace(LOG_AUDIO_CSL, "%s, ON=1,OFF=0 ?: %d\n",
		__func__, on);
	if(on)
