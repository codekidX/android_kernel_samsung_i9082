...
static int isTDModemCall = FALSE;
#endif
/*
...
#if defined(CONFIG_MACH_CAPRI_SS_BAFFIN_CMCC)
static void AUDCTRL_MicBiasOn(CSL_CAPH_DEVICE_e source);
static void AUDCTRL_ExtModem_DisableTelephony(void);
static void AUDCTRL_ExtModem_EnableTelephony(AUDIO_SOURCE_Enum_t source, AUDIO_SINK_Enum_t sink);
static void AUDCTRL_ExtModemSetTelephonyMicSpkr(AUDIO_SOURCE_Enum_t source, AUDIO_SINK_Enum_t sink, bool force);
//SPK -> LOUDMIC
#define Main_MIC   0
#define Loud_MIC   1
#define GPIO_MIC_SELECT 146
#endif
...
#else
	else if (sAudioAppStates[AUDIO_APP_EXTMODEM_VOICE_CALL])
		currAudioApp = AUDIO_APP_EXTMODEM_VOICE_CALL;
#endif
...
void AUDCTRL_SaveAudioApp(AudioApp_t app)
{
	...
	if (AUDCTRL_InVoiceCall())
		...
		if (app > AUDIO_APP_VOIP_INCOMM)
#else
		if ((app > AUDIO_APP_VOIP_INCOMM) && (app != AUDIO_APP_EXTMODEM_VOICE_CALL))
#endif
			return;
				...
				int arg1, int arg2, int arg3, int arg4)
{
	...
	case AUDCTRL_HW_CFG_MODE_SEL:
		...
		isTDModemCall = arg1;
		extern_set_TDCall(isTDModemCall);
		aTrace(LOG_AUDIO_CNTLR, "%s::"
			"isTDModemCall=%d \n",
			__func__, isTDModemCall);
		break;
	...
	case AUDCTRL_HW_WRITE_GAIN:
	   ...
	   path->srcmRoute[0][0].inChnl, outChnl, arg2, arg2); */
		...
		default:
			aTrace(LOG_AUDIO_CNTLR, "%s::No access index %d\n",__func__);
			break;
		}
		break;
#if defined(CONFIG_MACH_CAPRI_SS_BAFFIN_CMCC)
		case AUDCTRL_HW_EXTMODEM_CONTROL:
			{
				int extModem_enableCall = arg1;
				AUDIO_SOURCE_Enum_t source = arg2;
				AUDIO_SINK_Enum_t sink = arg3;

				aTrace(LOG_AUDIO_CNTLR, "%s::external modem control: %d:%d\n",
					__func__, AUDCTRL_HW_EXTMODEM_CONTROL, extModem_enableCall);

				switch (extModem_enableCall)
				{
					case 0:
						AUDCTRL_ExtModem_EnableTelephony(source, sink);
						break;
					case 1:
						AUDCTRL_ExtModem_DisableTelephony();
						break;
					case 2:
						AUDCTRL_ExtModemSetTelephonyMicSpkr(source, sink, false);
						break;
					default:
						aTrace(LOG_AUDIO_CNTLR, "%s::No access index %d\n", __func__, arg1);
						break;
...
static void AUDCTRL_ExtModemSetTelephonyMicSpkr(AUDIO_SOURCE_Enum_t source,
				 AUDIO_SINK_Enum_t sink,
				 bool force)
{
	...
	aTrace(LOG_AUDIO_CNTLR, "%s sink %d, source %d\n", __func__, sink, source);
	...
	app = AUDCTRL_GetAudioApp();
	if ((app > AUDIO_APP_VOICE_MAX) && (app != AUDIO_APP_EXTMODEM_VOICE_CALL))
		return;
	...
	if (sink == AUDIO_SINK_LOUDSPK){ //GPIO_START
		aTrace(LOG_AUDIO_CNTLR,  "Loud_MIC GPIO \n");
		gpio_request(GPIO_MIC_SELECT, "Loud_MIC enable");
		gpio_val = gpio_get_value(GPIO_MIC_SELECT);
		aTrace(LOG_AUDIO_CNTLR,  "Loud_MIC GPIO SPEAKER -> LOUDMIC gpio_val=%d, sink=%d\n", gpio_val, sink);
		gpio_direction_output(GPIO_MIC_SELECT, Loud_MIC);
		gpio_free(GPIO_MIC_SELECT);
	} else { //GPIO_STOP
		aTrace(LOG_AUDIO_CNTLR,  "Main_MIC GPIO \n");
		gpio_request(GPIO_MIC_SELECT, "Main_MIC enable");
		gpio_val = gpio_get_value(GPIO_MIC_SELECT);
		aTrace(LOG_AUDIO_CNTLR,  "Main_MIC GPIO SPEAKER -> MAINMIC gpio_val=%d, sink=%d\n", gpio_val, sink);
		gpio_direction_output(GPIO_MIC_SELECT, Main_MIC);
		gpio_free(GPIO_MIC_SELECT);
	}

	if(isTDModemCall)
	{
		aTrace(LOG_AUDIO_CNTLR, "%s isTDModemCall=%d ,TelephonyUse=%d\n", __func__,isTDModemCall,TelephonyUse);
		if (voiceCallSpkr != sink || force == true) {
...
static void AUDCTRL_ExtModem_EnableTelephony(AUDIO_SOURCE_Enum_t source, AUDIO_SINK_Enum_t sink)
{
	...
	aTrace(LOG_AUDIO_CNTLR, "%s isTDModemCall=%d \n", __func__,isTDModemCall);
	AUDCTRL_SaveAudioApp(AUDIO_APP_EXTMODEM_VOICE_CALL);
	mode = GetAudioModeBySink(sink);
	AUDCTRL_SaveAudioMode(mode);
	powerOnExternalAmp(sink, TelephonyUse, TRUE, FALSE);
	voiceCallSpkr = sink;
	voiceCallMic = source;
	AUDCTRL_MicBiasOn(source);
...
static void AUDCTRL_ExtModem_DisableTelephony(void)
{
	...
	powerOnExternalAmp(voiceCallSpkr, TelephonyUse,
			FALSE, FALSE);	
			AUDCTRL_RemoveAudioApp(AUDIO_APP_EXTMODEM_VOICE_CALL);

	isTDModemCall = FALSE;
	extern_set_TDCall(isTDModemCall);
	/*MAKE SURE SWITCHES ARE BACK TO BRCM MODEM CONTROL*/			
...
static void AUDCTRL_MicBiasOn(CSL_CAPH_DEVICE_e source)
{
	...
	aTrace(LOG_AUDIO_CNTLR, "MicBiasOn: source %d ", source);

	if (!bClk)
		csl_caph_ControlHWClock(TRUE);
	switch (source) {
	case AUDIO_SOURCE_ANALOG_MAIN:
		csl_caph_audioh_mic_bias_on(TRUE);
		break;
	case AUDIO_SOURCE_ANALOG_AUX:
		csl_caph_audioh_hs_mic_bias_on(TRUE);
		break;
	default:
		aTrace(LOG_AUDIO_CNTLR, "don't support: source %d ", source);
	}
